...
```
Steve              45 ms     ← зелёный: отличное соединение
Alex              137 ms     ← жёлтый: приемлемое
Herobrine         312 ms     ← красный: высокая задержка
```
...
1. Скачайте `PingDisplay-1.0.0.jar` со [страницы релизов](https://github.com/emptyenemy/ping_display/releases).
...
Готовый jar появится в `target/PingDisplay-1.0.0.jar`.

## Планы

Версия 1.0.0 — первый публичный релиз, минимальный по функциональности. В ближайших планах:

- поддержка актуальных версий Minecraft;
- перезагрузка конфига командой, без рестарта сервера;
- восстановление обычного ника в таб-листе при выключении плагина;
